...
namespace SocialOpinionAPI.Clients
{
    public class FilteredStreamClient
    {
        ...
        public object StartStream(string address, int maxTweets, int maxConnectionAttempts)
        {
                ...
                try
                {
                    ...
                    try
                    {
                        ...
                        if (response.StatusCode == HttpStatusCode.OK)
                        {
                            ...
                            using (StreamReader str = new StreamReader(response.GetResponseStream()))
                            {
                                ...
                                do
                                {
                                    ...
                                    if (!string.IsNullOrEmpty(json))
                                    {
                                        ...
                                        return ("records fetched:" + recordsFetch + " at " + DateTime.Now.ToString("F"));
                                    }
                            ...
                            return "Exited LabsStartFilteredStream at:" + DateTime.Now.ToString("F");
                        }
                        else
                        {
                            Console.WriteLine("response.StatusCode not HttpStatusCode.OK. Currently: " + response.StatusCode + " " + response.StatusDescription);
                            return("response.StatusCode not HttpStatusCode.OK. Currently: " + response.StatusCode + " " + response.StatusDescription);
                        }
                    }
